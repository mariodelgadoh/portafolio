...
// ===== CONTADOR DE VISITAS QUE SÍ FUNCIONA =====
function updateVisitCounter() {
    const counterElement = document.getElementById('visitCount');
    if (!counterElement) return;
    
    // Clave para saber si ya contó esta visita
    const SESSION_KEY = 'visit_counted_' + window.location.pathname;
    ...
    if (!visits) {
        // Primera vez: empezar con 128 o un número base
        visits = 128;
    } else {
        visits = parseInt(visits);
    }
    
    // Si no se ha contado esta sesión, aumentar
    if (!alreadyCounted) {
        visits = visits + 1;
    ...
    // Formatear el número con comas (ej: 1,234)
    counterElement.textContent = visits.toLocaleString();
    
    console.log('Visitas totales:', visits); // Para verificar en consola
}
